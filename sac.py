...
class Actor_Encoder(nn.Module):
    def __init__(self, input_dim, latent_dim, action_dim, encoder_weight_dir=None, freeze=False):
...
class QNetwork_Encoder(nn.Module):
    def __init__(self, state_dim, latent_dim, action_dim, encoder_weight_dir=None, freeze=False):
        super().__init__()
...
class SAC_RewardPrediction(SAC):
    def init_networks(self):
        self.q1 = QNetwork_Encoder(64, 64, self.action_dim, f'./Results/encoder/encoder_six.pth', True).to(device)
        self.q2 = QNetwork_Encoder(64, 64, self.action_dim, f'./Results/encoder/encoder_six.pth', True).to(device)

        self.q1_target = QNetwork_Encoder(64, 64, self.action_dim, f'./Results/encoder/encoder_six.pth', True).to(device)
        self.q2_target = QNetwork_Encoder(64, 64, self.action_dim, f'./Results/encoder/encoder_six.pth', True).to(device)
        self.q1_target.load_state_dict(self.q1.state_dict())
        self.q2_target.load_state_dict(self.q2.state_dict())
        self.actor = Actor_Encoder(64, 64, self.action_dim, './Results/encoder/encoder_six.pth', True).to(device)
        self.optim_q = torch.optim.Adam(list(self.q1.parameters()) + list(self.q2.parameters()), lr=0.001)
        self.optim_actor = torch.optim.Adam(self.actor.parameters(), lr=3e-4)

class SAC_RewardPredictionFinetune(SAC):
    def init_networks(self):
